...
export default function AttendanceEdit({ attendance, employees = [] }: PageProps) {
    const getTime = (dateTimeStr: string | null) => {
        if (!dateTimeStr) {
            return '';
        }
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        check_in: getTime(attendance.check_in),
        check_out: getTime(attendance.check_out),
        break_in: getTime(attendance.break_in),
        break_out: getTime(attendance.break_out),
    ...
    function handleSubmit(e: FormEvent) {
        ...
        ['check_in', 'check_out', 'break_in', 'break_out'].forEach((field) => {
            ...
            } else if (field !== '_method') {
                submissionData[field as keyof typeof submissionData] = null;
            }
        });

        post(EmployeeAttendanceController.update.url(attendance.id), {
        ...
        <div className="container mx-auto py-8 px-4 max-w-2xl space-y-6">
            ...
            <div className="flex items-center gap-3 border-b pb-5 dark:border-zinc-800">
                ...
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Update daily attendance details for the employee.</p>
                </div>
            </div>
