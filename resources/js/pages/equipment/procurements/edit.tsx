...
import type { Auth, EquipmentModel, EquipmentProcurement } from '@/types';
...
export default function EquipmentProcurementEdit({ equipmentProcurement, equipmentModels = [] }: PageProps) {
    const { data, setData, patch, processing, errors } = useForm({
        title: equipmentProcurement.title,
        description: equipmentProcurement.description || '',
        request_date: equipmentProcurement.request_date.split('T')[0],
        status: equipmentProcurement.status,
        items: equipmentProcurement.equipment_procurement_items?.map(item => ({
            id: item.id,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        patch(update.url(equipmentProcurement.id));
    };

    return (
        <>
            <Head title={`Edit Procurement ${equipmentProcurement.procurement_number}`} />

            <div className="flex flex-col gap-6 p-4 max-w-4xl mx-auto">
                <Heading
                    title={`Edit Request: ${equipmentProcurement.procurement_number}`}
                    description="Update your procurement request details."
                />
...
EquipmentProcurementEdit.layout = {
    breadcrumbs: [
