...
import { index, update } from '@/routes/event-registrations';
...
export default function EditEventRegistration({ eventRegistration, events = [], users = [], employees = [] }: PageProps) {
    const { data, setData, put, processing, errors } = useForm({
        event_id: eventRegistration.event_id,
        user_id: eventRegistration.user_id,
        registration_number: eventRegistration.registration_number,
        registered_at: formatDateForInput(eventRegistration.registered_at),
        attendance_status: eventRegistration.attendance_status,
        notes: eventRegistration.notes || '',
        ticket_number: eventRegistration.ticket_number,
        qr_code: eventRegistration.qr_code || '',
        issued_at: formatDateForInput(eventRegistration.issued_at),
        certificate_number: eventRegistration.certificate_number,
        file_path: eventRegistration.file_path || '',
        generated_by: eventRegistration.generated_by || '',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(update.url(eventRegistration.id));
    };
    ...
    return (
        <div className="container mx-auto py-8 px-4 max-w-2xl space-y-6">
            ...
            <div className="flex items-center gap-3 border-b pb-5 dark:border-zinc-800">
                <Link href={index.url({ event_id: data.event_id })}>
                ...
                <div>
                    <h1 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white flex items-center gap-2">
                        <Ticket className="w-6 h-6 text-violet-600 dark:text-violet-400" /> Edit Registration
                    </h1>
                    <p className="text-muted-foreground text-sm mt-1">
                        Modify participant details, ticket details, or registration status.
                    </p>
                </div>
            </div>
            ...
            <Card className="border dark:border-zinc-800">
                <CardHeader>
                    <CardTitle className="text-lg">Edit Registration Details</CardTitle>
                ...
                <CardContent>
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            ...
                            >
                                <Check className="w-4 h-4" /> Update Registration
                            </Button>
                        </div>
                    </form>
                </CardContent>
